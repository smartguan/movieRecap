...
class StoryTeller:
    ...
    def _build_hook(self, title: str, synopsis: str, genre: str, lead_char: str) -> str:
        ...
        if "诅咒" in title or "惊悚" in genre or "恐怖" in genre:
            return (
                f"社交网络上一则早已离世友人的异常更新，打破了都市日常的平静。"
                f"在悬疑惊悚电影《{title}》中，{lead_char}在日常工作中意外卷入一场顺着网络蔓延的夺命诅咒。"
                f"为了查明真相并拯救受牵连的身边人，她顺着跨洋线索踏上了一条追寻诅咒源头的惊险旅途。"
            )
        else:
            clean_syn = synopsis[:90] if synopsis else "一段跌宕起伏的传奇故事"
            return (
                f"看似平静的生活背后，往往潜藏着彻底改变命运轨迹的未知变故。"
                f"在电影《{title}》中，故事围绕着{clean_syn}展开，"
                f"主角在困境与迷局中艰难前行，逐步揭开一段掩藏已久的真相。"
            )
    ...
    def _build_conclusion(self, title: str, synopsis: str, genre: str, lead_char: str) -> str:
        ...
        if "诅咒" in title or "惊悚" in genre or "恐怖" in genre:
            return (
                f"随着施咒真凶被制伏归案，这场源于社交网络嫉恨的草人邪术终于告一段落。"
                f"历经生死波折的{lead_char}安葬了离世的挚友，将带有恶意的诅咒信物彻底销毁封存。"
                f"现实生活重归平静，但这场由虚荣与恶意引发的连环悲剧，却给每一个幸存者留下了难以磨灭的沉痛烙印。"
            )
        else:
            return (
                f"故事最终迎来了清晰的终局，所有悬念与矛盾在主角的抉择下得以化解。"
                f"经历了一连串波折与磨难，主角不仅找到了最初追寻的答案，也收获了内心的救赎与成长，为整段历程画上了平实的句号。"
            )
            ...
            act_definitions = [
                {
                    "act_id": 1,
                    "name": "序幕与异端初现",
                    "time_bounds": (0.0, min(850.0, total_duration_sec * 0.16)),
                    "lines": [
                ...
                },
                {
                    "act_id": 2,
                    "name": "诅咒蔓延与跨海追寻",
                    "time_bounds": (min(850.0, total_duration_sec * 0.16), min(3000.0, total_duration_sec * 0.53)),
                    "lines": [
                        f"次日清晨噩耗接踵而至，昨晚参与留言的第一位友人被发现离奇溺毙在自家浴缸中。",
                        f"警方现场勘验未发现任何外力入侵痕迹，但受害者的手心里却紧紧攥着半张烧焦的纸人。",
                        f"极度的恐慌在好友圈内彻底炸开，死神的镰刀正顺着互联网的数据流无情挥落。",
                        f"未等众人喘息，另一位同伴在众目睽睽之下精神失常，口中不断重复着无法理解的异国诅咒。",
                        f"{lead_char}的手机屏幕突然自行点亮，血红色的倒计时赫然显示只剩下最后的三天时间。",
                        f"眼看着身边的朋友相继死于非命，她明白坐以待毙只会走向全军覆没的结局。",
                        f"她翻箱倒柜找到了好友生前从台湾寄回的所有明信片与未寄出的随笔日记。",
                        f"字里行间隐晦地透露出，好友在台湾求学期间曾意外涉足了一场禁忌的民俗仪式。",
                        f"为了拯救身边的同伴并斩断夺命的诅咒锁链，女主背起行囊，毅然登上了飞往台北的航班。",
                        f"飞机穿破浓密的雷雨云层，窗外电闪雷鸣，预示着异乡等待她的将是一场生死浩劫。",
                        f"走出机场大厅，湿热黏稠的空气扑面而来，周围嘈杂的人声却无法驱散心头的阴霾。",
                        f"她按照日记中的联系方式，找到了好友生前在当地的唯一密友{friend_char}。",
                    ],
                },
                {
                    "act_id": 3,
                    "name": "迷雾重重与民俗旧案",
                    "time_bounds": (min(3000.0, total_duration_sec * 0.53), min(4400.0, total_duration_sec * 0.77)),
                    "lines": [
                ...
                },
                {
                    "act_id": 4,
                    "name": "深入禁地与绝命搏杀",
                    "time_bounds": (min(4400.0, total_duration_sec * 0.77), min(5000.0, total_duration_sec * 0.88)),
                    "lines": [
                ...
                },
                {
                    "act_id": 5,
                    "name": "怨气消散与黎明救赎",
                    "time_bounds": (min(5000.0, total_duration_sec * 0.88), total_duration_sec),
                    "lines": [
            ...
            act_definitions = [
                {
                    "act_id": 1,
                    "name": "序幕与伏笔",
                    "time_bounds": (0.0, total_duration_sec * 0.20),
                    "lines": [
                        f"故事拉开帷幕，主要人物依次登场，在看似平常的日常生活中悄然埋下了不安的伏笔。",
                        f"随着一段意想不到的突发事件爆发，原有的平静轨迹被彻底打破，主角被迫直面命运的考验。",
                        f"周围的环境逐渐被疑云笼罩，看似偶然的巧合背后隐藏着错综复杂的因果纠缠。",
                    ],
                },
                {
                    "act_id": 2,
                    "name": "矛盾展开与危机调查",
                    "time_bounds": (total_duration_sec * 0.20, total_duration_sec * 0.50),
                    "lines": [
                ...
                },
                {
                    "act_id": 3,
                    "name": "转折与深入腹地",
                    "time_bounds": (total_duration_sec * 0.50, total_duration_sec * 0.75),
                    "lines": [
                ...
                },
                {
                    "act_id": 4,
                    "name": "生死抉择与巅峰对决",
                    "time_bounds": (total_duration_sec * 0.75, total_duration_sec * 0.90),
                    "lines": [
                        f"故事进入全片最为紧张窒息的高潮阶段，正邪交锋在千钧一发之际全面爆发。",
                        f"绝境之下退无可退，主角不得不赌上一切，迎战最为残酷的终极考验。",
                        f"主角在生死攸关的绝境中完成了关键破局，用坚定的意志扭转了看似注定的败局。",
                    ],
                },
                {
                    "act_id": 5,
                    "name": "尘埃落定与主题升华",
                    "time_bounds": (total_duration_sec * 0.90, total_duration_sec),
                    "lines": [
                        f"决战过后尘埃落定，真相终于在晨曦中大白于天下，危机得以彻底化解。",
                        f"劫后余生的人物相继迎来了各自的归宿，过去的伤痛也在这场洗礼中得到治愈。",
                        f"影片在深沉厚重的情感余波中缓缓收尾，为观众留下了关于人性与勇气的持久反思。",
                    ],
                },
            ]

        # Allocate target segments across acts proportionally
        total_act_lines = sum(len(a["lines"]) for a in act_definitions)
        raw_allocations = [
            max(1, round((len(a["lines"]) / total_act_lines) * num_segments))
            for a in act_definitions
        ]
        # Adjust sum of allocations to match num_segments exactly
        diff = num_segments - sum(raw_allocations)
        if diff != 0:
            raw_allocations[2] = max(1, raw_allocations[2] + diff)

        for act_idx, act in enumerate(act_definitions):
            act_n = raw_allocations[act_idx]
            act_lines = act["lines"]
            act_st, act_et = act["time_bounds"]
            act_dur = max(10.0, act_et - act_st)

            # Pick lines from act_lines
            if act_n <= len(act_lines):
                step = len(act_lines) / float(act_n)
                selected_lines = [act_lines[int(j * step)] for j in range(act_n)]
            else:
                selected_lines = list(act_lines)
                extra = act_n - len(act_lines)
                for k in range(extra):
                    insert_pos = int((k + 1) * (len(selected_lines) / (extra + 1)))
